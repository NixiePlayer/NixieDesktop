...
export interface BrowserAccount {
	 ...
	 * owning its YouTube cookies, so nothing may present it as the account Nixie will use.
	 */
	accountEmail?: string;
...
/**
 * A browser profile that has connected through the Nixie extension, as the sign-in screen shows it.
 * The disk-read path off macOS is dead for app-bound Chromium, so this is how a Windows Chrome reaches
 * the app: the extension reads its own cookies and the app pulls them through the native host.
 */
export interface ExtensionSource {
	/** Opaque, generated by the extension. The only thing that tells two profiles of one browser apart. */
	installId: string;
