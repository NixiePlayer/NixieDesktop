...
describe("Explore shelves", () => {
	...
	it("forwards the root and destination browse arguments exactly", async () => {
		...
		const adapter = new YouTubeAdapter(
			...
			join(tmpdir(), "noctune-cache-missing"),
...
describe("home shelves", () => {
	...
	const homeAdapter = (execute: ReturnType<typeof vi.fn>, cookie: string) => {
		vi.mocked(Innertube.create).mockResolvedValueOnce({ actions: { execute } } as never);
		return new YouTubeAdapter({ registerArtwork: identity } as never, join(tmpdir(), "noctune-cache-missing"), () =>
...
describe("entity extraction", () => {
	const items = extractEntities(shelf, (url) => `noctune://app/artwork/${encodeURIComponent(url)}`);
	...
	it("reads the fields upstream actually exposes", () => {
		...
		expect(items[0]).toHaveProperty("artworkUrl", expect.stringContaining("noctune://app/artwork/"));
...
describe("album header", () => {
	...
	const items = withAlbumHeader("MPREb_album", header, rows, (url) => `noctune://app/artwork/${url}`);
	...
	it("turns the header into the page's album", () => {
		expect(items[0]).toEqual({
			...
			year: "2004",
			artworkUrl: "noctune://app/artwork/https://example.test/cover.jpg",
			kind: "Album",
			explicit: true,
		});
	});
...
describe("playlist header", () => {
	...
	const items = withPlaylistHeader("VLPLcool", source, [], (url) => `noctune://app/artwork/${url}`);
	...
	it("replaces auto-generated metadata that points at settings Noctune does not expose", () => {
...
describe("artist header", () => {
	...
	const items = withArtistHeader("UC_blink_182", header, related, (url) => `noctune://app/artwork/${url}`);
	...
	it("turns the header into the page's artist", () => {
		expect(items[0]).toEqual({
			...
			artworkUrl: "noctune://app/artwork/https://example.test/blink=w544-h544",
			bannerUrl: "noctune://app/artwork/https://example.test/blink=w1920-h1080",
...
describe("session cookies", () => {
	it("builds a new session once the copied cookies rotate", async () => {
		...
		create.mockClear();
		const adapter = new YouTubeAdapter({} as never, join(tmpdir(), "noctune-cache-missing"), () =>
...
describe("account", () => {
	it("reads the selected channel and its largest photo", async () => {
		...
		const adapter = new YouTubeAdapter(
			{ registerArtwork: (url: string) => `noctune://app/artwork/${url}` } as never,
			join(tmpdir(), "noctune-cache-missing"),
			() => Promise.resolve("SID=first")
		);
		expect(await adapter.account()).toEqual({
			accountName: "Listener",
			avatarUrl: "noctune://app/artwork/https://example.test/s176.jpg",
		});
	});
});
...
describe("YouTube parser cache", () => {
	it("keeps newest files within its byte limit", async () => {
		const path = await mkdtemp(join(tmpdir(), "noctune-cache-"));
		paths.push(path);
