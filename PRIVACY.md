...
Signing in copies the YouTube cookies out of a browser profile you pick. Google expires that session every few minutes and only the browser holds the current value, so Nixie reads the same profile again while it runs, at most once a minute, for as long as the account stays linked. It reads the YouTube cookies and nothing else, it names the profile in a file in its own directory so it knows which one to read, and it writes nothing back to the browser. Signing out ends it. Those cookies are sent to YouTube, which is what makes the session work, and to nowhere else.
