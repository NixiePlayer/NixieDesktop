...
writeFileSync(join(root, "node_modules", "electron", "path.txt"), "Nixie.app/Contents/MacOS/Electron");

const plist = join(to, "Contents", "Info.plist");
// The bundle path stays `Nixie.app` because `path.txt` above points at it, but the two name keys
// carry the channel, so a development window is never mistaken for the installed app beside it.
for (const key of ["CFBundleName", "CFBundleDisplayName"]) {
	execFileSync("plutil", ["-replace", key, "-string", "Nixie (Dev)", plist]);
}
