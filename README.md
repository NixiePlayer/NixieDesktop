...
> [!IMPORTANT]
> **Nixie is beta software, and macOS only for now.** Things move, and some of them
...
| File | For |
| --- | --- |
| `Nixie-<version>-applesilicon.dmg` | M1 and later |
...
platform, and Nixie reads that profile's YouTube cookies. They go into a dedicated
...
every few minutes and only the browser holds the current value, so Nixie re-reads that
...
The app opens with a blue icon and calls itself Nixie (Dev). Sign in by picking a
