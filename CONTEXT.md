...
- **Noctune**: the local Electron application.
- **InnerTube**: YouTube's private, undocumented application API.
- **Website session**: YouTube cookies stored only in Electron's `persist:noctune-auth` partition.
- **Media token**: a random, short-lived identifier resolved by `noctune://app/media/<token>`.
