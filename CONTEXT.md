...
- **Neotune**: the local Electron application.
- **InnerTube**: YouTube's private, undocumented application API.
- **Website session**: YouTube cookies stored only in Electron's `persist:neotune-auth` partition.
- **Media token**: a random, short-lived identifier resolved by `neotune://app/media/<token>`.
...
Neotune uses pinned `youtubei.js` 17.2.0 in Electron's main process. The renderer receives plain DTOs, opaque continuation values, proxied artwork, and opaque media tokens. Parser objects, cookies, signed URLs, and arbitrary upstream URLs stay in main.
